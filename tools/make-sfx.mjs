...
const ms = (t) => Math.max(1, Math.round(RATE * t / 1000));
...
const square = (p) => (p < 0.5 ? 1 : -1);
const saw = (p) => p * 2 - 1;
const tri = (p) => 4 * Math.abs(p - 0.5) - 1;
...
const decay = (t, tau) => Math.exp(-t / (tau / 1000));

//  Exponential frequency glide from f0 to f1 across dur ms.
const glide = (t, dur, f0, f1) => f0 * Math.pow(f1 / f0, Math.min(t / (dur / 1000), 1));

//  Soft saturation. Adds weight without the crackle of hard clipping.
const sat = (x, drive = 1) => Math.tanh(x * drive);

//  ------------------------------------------------------------------ pipeline

function render (durMs, fn)
{
    const len = ms(durMs);
...
function declick (buf, inMs = 1, outMs = 4)
{
    const a = ms(inMs);
    const b = ms(outMs);
